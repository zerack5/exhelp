...
data2co <- read.xlsx('C:/Users/a/Desktop/数据分析/wsh/GSE185694_fpkm_1.xlsx', colNames = TRUE,rowNames = TRUE)
sdata2co <- data2co[c(grep("Con",colnames(data2co)))]
sdata2co <- sdata2co[-c(grep("FPKM",colnames(sdata2co)))]

data2fe <- read.xlsx('C:/Users/a/Desktop/数据分析/wsh/GSE185694_fpkm_1.xlsx', colNames = TRUE,rowNames = TRUE)
sdata2fe <- data2fe[c(grep("DFO",colnames(data2fe)))]
...
##载入样本信息
fzdata2 <- read.csv('wshfe1注释.csv', sep = ',',header = T,row.names = 1)
...
sfzdata2[,2] <- as.factor(sfzdata2$condition)


all(rownames(sfzdata2) %in% colnames(count))
all(rownames(sfzdata2) == colnames(count))

dds <-  DESeqDataSetFromMatrix(countData = count,colData = sfzdata2,design = ~condition)
dim(dds)
...
write.csv(diff,"all_diff_DFO.csv")
